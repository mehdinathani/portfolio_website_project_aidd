---
	id: 0014
	title: fix-frontend-build-and-admin-pages
	stage: green
	date: 2026-05-01
	surface: agent
...
11. **Added `export const dynamic = 'force-dynamic'`** to all admin pages so they are not statically prerendered

## Outcome

- ✅ Impact: Fixed 16+ frontend files with syntax errors, created middleware + api-admin.ts, fixed env var references, added missing files (error.tsx, sitemap.xml, next.config.js), updated types
- 🧪 Tests: none
- 📁 Files: 16 files modified/created across frontend/src and frontend/public
- 🔁 Next prompts: Build the frontend successfully with `npm run build`, then verify all public pages render with data from backend
...
- Failure modes observed: Original files had `export const dynamic = 'force-dynamic'` placed inside function bodies causing syntax errors. The regex-based refactoring left stray backticks and broken function references. The middleware used an invalid regex pattern for the matcher config.
- Graders run and results (PASS/FAIL): Build compiles successfully through webpack step; prerendering fails for admin pages without env vars (expected).
